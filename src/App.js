...
import { Container, Navbar, Nav, Modal, Button, Form, Table } from "react-bootstrap";
...
function App() {
  ...
  const deleteContest = async (id) => {
    ...
    alert("Contest was successfully deleted");
  }
  useEffect(() => {
    const getcontests = async () => {
      const data = await getDocs(contestsCollectionRef);
      setcontests(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
    getcontests();
  }, []);
  return (
          ...
          <tbody>
            {contests.map(contest => {
              return <tr>
                ...
                <td>{contest.date}</td>
                <td>{contest.time}</td>
                <td><Button className="btn-sm" onClick={() => { deleteContest(contest.id) }}>Delete</Button></td>
              </tr>
            })}
          </tbody>
        </Table>
